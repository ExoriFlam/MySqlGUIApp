...
TableInsertList::TableInsertList(int x, int y, int w, int h, std::shared_ptr<EventSystem> e_sys):
	...
	lbale_auto_inc(0, 0, 0, 0), save_btn(0, 0, 0, 0), insert_rows_list(0, 0, 0, 0) 
{
	(void)e_sys;
	insert_rows_list.end();
    ...
    insert_rows_list.resize(x + 5, y + 35, w - 10, 380);
    insert_rows_list.type(Fl_Scroll::BOTH);

    //insert_rows_list.color(FL_WHITE);
